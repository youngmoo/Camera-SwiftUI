...
public class CameraService : NSObject {
    ...
    // Adding this member so we can set videoOrientation after the session has been configured
    ...
    private func configureSession() {
        ...
        // For YouTube playback (within WKWebView), need to set AVAudioSession options to [.mixWithOthers] in order to hear video
        ...
        do {
            ...
            print("Couldn't create audio/video device input(s): \(error)")
            setupResult = .configurationFailed
            session.commitConfiguration()
            return
        }
    ...
    private func configureMovieFileOutput() {
        ...
        guard session.canAddOutput(movieFileOutput!) else {return}
        
        session.addOutput(movieFileOutput!)
        
        let videoConnection = movieFileOutput!.connection(with: AVMediaType.video)
        ...
        if self.setupResult != .configurationFailed {
            ...
            sessionQueue.async {
                if self.movieFileOutput!.isRecording {
                    ...
                    self.movieFileOutput!.startRecording(to: fileUrl, recordingDelegate: self as AVCaptureFileOutputRecordingDelegate)
                }
            }
        }
        /*
